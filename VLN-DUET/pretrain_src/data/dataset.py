...
class ReverieTextPathData(object):
    def __init__(
        ...
        max_txt_len=100, in_memory=True, act_visited_node=False, aug_prob=0.5, ft_type="ori"
    ):
        ...
        for i, anno_file in enumerate(anno_files):
            with jsonlines.open(anno_file, 'r') as f:
                if ft_type == "random":
                    ...
                    if "inpaint" in anno_file:
                        # assert (i-1) == count
                        ft = "inpaint"+"_"+str(i-1)
                    else:
                        ft = "ori"
                for item in f:
                    item["ft_type"] = ft
        ...
        if len(self.img_ft_file) > 1:
            ...
            for img_file in self.img_ft_file[1:]:
                aug_feat_dict = dict()
                with open(img_file, 'r') as f:
                    ...
                    for item in reader:
                        ...
                        vp = item["viewpointId"]
                        key = '%s_%s' % (scan, vp)
                        aug_feat_dict[key] = np.frombuffer(base64.b64decode(item['features']),
                                                           dtype=np.float32).reshape((36, image_feat_size))
                self._feature_store_aug.append(aug_feat_dict)

    def __len__(self):
        return len(self.data)

    def get_scanvp_feature(self, scan, viewpoint, ori=False, ft_type="ori"):
        if ft_type == "ori":
            ori = True
        ...
        if not ori and self._feature_store_aug is not None and random.random() < self.aug_prob:
            index = random.randrange(len(self._feature_store_aug))
            view_fts[:,:self.image_feat_size] = self._feature_store_aug[index][key]

        if "inpaint" in ft_type:
            index = int(ft_type.split("_")[-1])
            view_fts[:,:self.image_feat_size] = self._feature_store_aug[index][key]
    ...
    def get_input(
        ...
        if return_act_label:
            idx = idx % self.ori_data

        item = self.data[idx]
        scan = item['scan']
        start_vp = item['path'][0]
        start_heading = item.get('heading', 0)
        if 'pos_vps' not in item:
            item['pos_vps'] = [item['path'][-1]]
            # print(item['pos_vps'])
            # print(item['path'])
        pos_vps = item['pos_vps']
        gt_path = item['path']
...
class R2RTextPathData(ReverieTextPathData):
    def __init__(
        ...
        image_feat_size=2048, image_prob_size=1000, angle_feat_size=4,
        max_txt_len=100, in_memory=True, act_visited_node=False, aug_prob=0.3, ft_type="ori"
    ):
        super().__init__(
            ...
            act_visited_node=act_visited_node, aug_prob=aug_prob, ft_type=ft_type
        )
    ...
    def get_input(
        ...
        traj_view_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
            last_vp_angles = self.get_traj_pano_fts(scan, gt_path, ft_type=item["ft_type"])
    ...
    def get_traj_pano_fts(self, scan, path, ft_type="ori"):
        '''
        Tokens in each pano: [cand_views, noncand_views, objs]
        Each token consists of (img_fts, loc_fts (ang_fts, box_fts), nav_types)
        '''
        ...
        for vp in path:
            if vp == path[-1]:
                view_fts = self.get_scanvp_feature(scan, vp, ori=True, ft_type=ft_type)
            else:
                view_fts = self.get_scanvp_feature(scan, vp, ft_type=ft_type)
